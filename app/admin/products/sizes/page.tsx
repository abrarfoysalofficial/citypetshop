...
export default function ProductSizesPage() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/product-sizes");
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = editing
        ? await fetch("/api/admin/product-sizes", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editing, ...data }) })
        : await fetch("/api/admin/product-sizes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
  ...
  const onDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/product-sizes?id=${id}`, { method: "DELETE" });
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-slate-900">Add Product SIZE</h1>
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-wrap gap-4">
          <input {...register("value")} placeholder="Value (e.g. Large)" className="rounded-lg border border-slate-300 px-3 py-2" />
